 ...
 * PIDOutput
 */
public class PIDControllerOverrideableError implements IUtility, LiveWindowSendable, Controller {
    ...
    private class PIDTask extends TimerTask {
        ...
        private PIDControllerOverrideableError m_controller;

        public PIDTask(PIDControllerOverrideableError controller) {
            if (controller == null) {
     ...
     */
    public PIDControllerOverrideableError(double Kp, double Ki, double Kd, double Kf,
     ...
     * @param period 
     */
    public PIDControllerOverrideableError(double Kp, double Ki, double Kd,
            PIDSource source, PIDOutput output,
            double period) {
    ...
    public double calculateError(double target, double position) {
        ...
        if (m_continuous) {
            if (Math.abs(result)
                ...
                if (result > 0) {
                    result = result - m_maximumInput + m_minimumInput;
                } else {
                    result = result
                            + m_maximumInput - m_minimumInput;
                }
    ...
    protected void calculate() {
        ...
        if (enabled) {
            ...
            synchronized (this) {
                m_error = calculateError(m_setpoint, input);
    ...
    public synchronized double getError() {
        //return m_error;
        return calculateError(getSetpoint(), m_pidInput.pidGet());
    }
